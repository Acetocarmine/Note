...
静态NAT（Static NAT）：每个私有IP地址映射到一个唯一的公共IP地址。这种方式适合需要外部设备直接访问的内部设备，如Web服务器。

动态NAT（Dynamic NAT）：私有IP地址池中的地址动态映射到公共IP地址池中的地址，且映射是临时的。适用于不需要特定公共IP地址的内部设备。

端口地址转换（PAT，Port Address Translation）或NAT重载：多台设备共享一个公共IP地址，通过不同的端口号来区分每个设备的连接。它是最常见的NAT形式，广泛用于家庭和小型企业网络。